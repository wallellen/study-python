        ...
        try:
            with open('test.dat', 'wb') as nf:
                with zipfile.ZipFile('test.zip', 'w', zipfile.ZIP_DEFLATED) as zf:
                    nf.write(data)
                    zf.writestr('test.dat', bytes(data))    # byte(bytearray), convert bytearray object to byte array
    ...
    def test_zip_file_py3(self):
        ...
        try:
            with open('test.dat', 'wb') as nf:
                with zipfile.ZipFile('test.zip', 'w', zipfile.ZIP_DEFLATED) as zf:
                    nf.write(data)
                    zf.writestr('test.dat', data)  # in python3, writestr support bytearray object as parameter
